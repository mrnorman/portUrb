      ...
      for (int iturb = 0; iturb < turbine_group.turbines.size(); iturb++) {
        ...
        if (turbine.active) {
          ...
          real2d umag_19_5m_2d("umag_19_5m_2d",ny,nx);
          {
            ...
            parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<3>(num_z,num_y,num_x) , KOKKOS_LAMBDA (int k, int j, int i) {
            ...
            parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<2>(ny,nx) , KOKKOS_LAMBDA (int j, int i) {
              real x = (i_beg+i+0.5_fp)*dx;
              real y = (j_beg+j+0.5_fp)*dy;
