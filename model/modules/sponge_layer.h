...
namespace modules {
  ...
  inline void sponge_layer( core::Coupler &coupler , real dt , real time_scale , real top_prop ) {
    ...
    if (!dm.entry_exists("sponge_u_col")) {
      ...
      dm.register_and_allocate<real>("sponge_T_col","",{nz});
      auto u_col = dm.get<real,1>("sponge_u_col");
      auto v_col = dm.get<real,1>("sponge_v_col");
      auto T_col = dm.get<real,1>("sponge_T_col");
      ...
      coupler.get_parallel_comm().all_reduce( T_col , MPI_SUM , "" ).deep_copy_to(T_col);
    }
    auto u_col = dm.get<real,1>("sponge_u_col");
    auto v_col = dm.get<real,1>("sponge_v_col");
    auto T_col = dm.get<real,1>("sponge_T_col");
    ...
    parallel_for( YAKL_AUTO_LABEL() , SimpleBounds<3>(nz,ny,nx) , KOKKOS_LAMBDA (int k, int j, int i) {
      ...
      if (z > z1) {
        ...
        dm_T(k,j,i) = factor*T_col(k) + (1-factor)*dm_T(k,j,i);
      }
    });
  }
