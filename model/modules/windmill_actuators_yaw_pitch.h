...
namespace modules {
  ...
  struct WindmillActuators {
    ...
    real1d ref_vel_inf_mag; // Velocity magnitude at infinity (m/s)
    real1d ref_thrust_coef; // Thrust coefficient             (dimensionless)
    real1d ref_power_coef;  // Power coefficient              (dimensionless)
    real1d ref_power;       // Power generation               (MW)
    real1d ref_pitch;       // Pitch                          (radians)
    real   ref_yaw_speed;   // Angular active yawing speed    (radians / sec)
    real   ref_pitch_speed; // Angular active yawing speed    (radians / sec)

    struct Turbine {
      real              base_location_x;  // x location of the tower base
      real              base_location_y;  // y location of the tower base
      ...
      real1d            hub_vel_trace;    // Time trace of velocities at the hub (time_dim)
      std::vector<real> hub_power_trace;  // Time trace of power in the hub (time_dim)
      real              yaw_angle;        // Current yaw angle   (radians going clockwise from facing west)
      real              pitch_angle;      // Current pitch angle (radians going backward from vertical)
    };
