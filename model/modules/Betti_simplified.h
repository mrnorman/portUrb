...
namespace modules {
  ...
  struct Floating_motions_betti {
    real static constexpr dt_max = 0.005;
    realHost1d        ref_vel;
    realHost1d        ref_CP;
    realHost1d        ref_CT;
    ...
    void init( realHost1d ref_vel , realHost1d ref_CP , realHost1d ref_CT , size_t rand_seed = 0 ) {
    ...
    structure( SArray<real,1,6> const & x_1 , real t , real turbine_wind , real wind_19_5m ) {
      ...
      // Wind Force
      real v_in      = turbine_wind + v_zeta + d_P*omega*std::cos(alpha);
      auto [Cp,Ct]   = CpCt( v_in );
      real FA        = 0.5*rho*A*Ct*v_in*v_in;
      real FAN       = 0.5*rho*C_dN*A_N    *std::cos(alpha)*std::pow(turbine_wind+v_zeta+d_N*omega*std::cos(alpha),2._fp);
      real FAT       = 0.5*rho*C_dT*h_T*D_T*std::cos(alpha)*std::pow(turbine_wind+v_zeta+d_T*omega*std::cos(alpha),2._fp);
