...
namespace modules {
  ...
  struct Dynamics_Euler_Stratified_WenoFV {
    ...
    void time_step_sspab3( core::Coupler & coupler ,
      ...
      parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<4>(num_state+num_tracers,nz,ny,nx) ,
    ...
    void compute_tendencies( core::Coupler       & coupler      ,
      ...
      parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<4>(num_state+num_tracers+1,nz,ny,nx) ,
                                        KOKKOS_LAMBDA (int l, int k, int j, int i) {
      ...
      parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<4>(mx,nz,ny,nx) , KOKKOS_LAMBDA (int l, int k, int j, int i) {
        if (l < num_state) {
    ...
    void halo_boundary_conditions( core::Coupler const & coupler ,
      ...
      if (coupler.get_option<std::string>("bc_x") == "precursor" && coupler.get_px() == 0) {
        parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<4>(num_state+num_tracers+1,nz,ny,hs) ,
      ...
      if (coupler.get_option<std::string>("bc_y") == "precursor" && coupler.get_py() == 0) {
        parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<4>(num_state+num_tracers+1,nz,hs,nx) ,
      ...
      // z-direction BC's
      if (bc_z == "solid_wall") {
        parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<4>(num_state+num_tracers+1,hs,ny,nx) ,
    ...
    void edge_exchange( core::Coupler const & coupler  ,
      ...
      // x-exchange
      {
        ...
        parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<3>(npack,nz,ny) , KOKKOS_LAMBDA (int v, int k, int j) {
      ...
      // y-direction exchange
      {
        ...
        parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<3>(npack,nz,nx) , KOKKOS_LAMBDA (int v, int k, int i) {
      ...
      if (coupler.get_option<std::string>("bc_x") == "precursor" && coupler.get_px() == 0) {
        parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<3>(num_state+num_tracers+1,nz,ny) ,
      ...
      if (coupler.get_option<std::string>("bc_y") == "precursor" && coupler.get_py() == 0) {
        parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<3>(num_state+num_tracers+1,nz,nx) ,
      ...
      if (bc_z == "solid_wall") {
        parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<3>(npack,ny,nx) , KOKKOS_LAMBDA (int l, int j, int i) {
          if (l == idW || l == idT) {
    ...
    void init(core::Coupler &coupler) const {
      ...
      parallel_for( YIKL_AUTO_LABEL() , nz+2*hs , KOKKOS_LAMBDA (int k) {
        for (int j = 0; j < ny; j++) {
          for (int i = 0; i < nx; i++) {
      ...
      parallel_for( YIKL_AUTO_LABEL() , hs , KOKKOS_LAMBDA (int kk) {
        {
          int  k0       = hs;
          int  k        = k0-1-kk;
          real rho0     = r(k0);
      ...
      auto create_immersed_proportion_halos = [] (core::Coupler &coupler) {
        ...
        if (!dm.entry_exists("dycore_immersed_proportion_halos")) {
...

          {
            ...
            parallel_for( YIKL_AUTO_LABEL() , SimpleBounds<3>(hsnew,ny+2*hsnew,nx+2*hsnew) ,
      ...
      auto compute_hydrostasis_edges = [] (core::Coupler &coupler) {
        ...
        if (ord < 5) {
          parallel_for( YIKL_AUTO_LABEL() , nz+1 , KOKKOS_LAMBDA (int k) {
      ...
      coupler.register_write_output_module( [=] (core::Coupler &coupler, yakl::SimplePNetCDF &nc) {
        ...
        auto hy_dens_cells = dm.get<float const,1>("hy_dens_cells");
        yikl::parallel_for( YIKL_AUTO_LABEL() , yikl::Bounds<3>(nz,ny,nx) , KOKKOS_LAMBDA (int k, int j, int i) {
