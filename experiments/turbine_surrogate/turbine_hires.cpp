...
int main(int argc, char** argv) {
  ...
  yakl::init();
  {
    ...
    int sp_cells = std::max(5,(int)0.05*nx_glob);
    ...
    while (etime < sim_time) {
      ...
      // Run modules
      {
        ...
        auto run_sf     = [&] (Coupler &coupler) { modules::apply_surface_fluxes(coupler,dtphys);            };
        auto run_turb   = [&] (Coupler &coupler) { windmills.apply              (coupler,dtphys);            };
        auto run_edge   = [&] (Coupler &coupler) { edge_sponge.apply            (coupler,dtphys,dtphys*100,sp_cells,sp_cells,0,0,10); };
        ...
        coupler.run_module( run_sf     , "surface_fluxes   " );
