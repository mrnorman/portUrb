...
nc1 = Dataset("abl_neutral_10m_00000032.nc","r")
nc2 = Dataset("abl_neutral_10m_00000040.nc","r")
nz = nc1.dimensions["z"].size
ny = nc1.dimensions["y"].size
nx = nc1.dimensions["x"].size
x = nc1.variables["x"][:]/1000.
y = nc1.variables["y"][:]/1000.
z = nc1.variables["z"][:]/1000.

yind = int(ny/2)
z2   = int(750./1200.*nz)
u = nc2.variables["uvel"][:z2,yind,:,0]
v = nc2.variables["vvel"][:z2,yind,:,0]
w = nc2.variables["wvel"][:z2,yind,:,0]
mag = np.sqrt(u*u+v*v+w*w)
X,Z = np.meshgrid(x,z[:z2])
CS = plt.contourf(X,Z,mag,levels=100,cmap="turbo",extend="both")
plt.title(f"Wind Speed at y={y[yind]}km")
plt.axis('scaled')
plt.margins(x=0)
plt.tight_layout()
...
plt.savefig("abl_neutral_wind_countour_xz.png",dpi=300)
plt.show()
plt.close()

zind = int(80./1200.*nz)
u = nc2.variables["uvel"][zind,:,:,0]
v = nc2.variables["vvel"][zind,:,:,0]
w = nc2.variables["wvel"][zind,:,:,0]
mag = np.sqrt(u*u+v*v+w*w)
X,Y = np.meshgrid(x,y)
CS = plt.contourf(X,Y,mag,levels=100,cmap="turbo",extend="both")
plt.title(f"Wind Speed at y={y[yind]}km")
plt.axis('scaled')
plt.margins(x=0)
plt.tight_layout()
...
cbar = plt.colorbar(CS,orientation="horizontal",cax=cax)
cbar.ax.tick_params(rotation=45)
plt.savefig("abl_neutral_wind_countour_xy.png",dpi=300)
plt.show()
plt.close()


z2 = int(1000./1200.*nz)
u     = nc2.variables["uvel"][:z2,:,:,0]
v     = nc2.variables["vvel"][:z2,:,:,0]
w     = nc2.variables["wvel"][:z2,:,:,0]
tke_u = nc2.variables["TKE" ][:z2,:,:,0]
mag = np.sqrt(u*u+v*v+w*w)
...
z1 = int(100./1200.*nz)
z2 = int(400./1200.*nz)
u  = nc2.variables["uvel"][z1:z2,:,:,0]
v  = nc2.variables["vvel"][z1:z2,:,:,0]
w  = nc2.variables["wvel"][z1:z2,:,:,0]
freq,spd = spectra((u*u+v*v+w*w)/2,1.02,50,dx)
plt.loglog( freq , spd*freq**(5/3) , label="KE")
plt.xlabel(r"$Frequency$")
plt.ylabel(r"$E(K) \omega^{5/3}$ (Compensated KE Spectra) ")
plt.legend()
plt.savefig("abl_neutral_ke_spectra.png",dpi=300)
plt.show()
plt.close()
